...
\pause
\begin{itemize}
\item Для беззнаковых: \begin{math}[0, 2^{bits}-1] \rightarrow [0, 1]\end{math} по формуле \mintinline{cpp}|x / (pow(2, bits) - 1)|
\pause
\item Для знаковых: \begin{math}[-2^{bits-1}, 2^{bits-1}-1] \rightarrow [-1, 1]\end{math} по формуле \mintinline{cpp}|max(-1, x / (pow(2, bits-1) - 1))|
\end{itemize}
\pause
