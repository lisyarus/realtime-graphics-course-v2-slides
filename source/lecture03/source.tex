...
\usemintedstyle{solarized-light}
\begin{itemize}
\item Можно получить виртуальный указатель на буфер или его часть и пользоваться им для чтения/записи: \mintinline{rust}|buffer.mapAsync(mode, offset, size)|
\pause
\begin{itemize}
...
\end{itemize}
\pause
\item Эта операция \textit{асинхронная}: принимает \textit{callback}, который будет вызван, когда результат будет готов
\pause
...
\usemintedstyle{solarized-light}
\begin{itemize}
\item Для рендеринга нужно выставить буфер с вершинами через \mintinline{rust}|render_pass.set_vertex_buffer(slot, buffer)|
\pause
...
\pause
\item \mintinline{rust}|render_pass.set_index_buffer(buffer, format)| устанавливает текущий буфер с индексами
\pause
