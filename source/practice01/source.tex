...
\item Из неё вам будут нужны \mintinline{cpp}|app.device|, \mintinline{cpp}|app.queue|, \mintinline{cpp}|app.surface_format|
...
\item На C++ их лучше инициализировать специальными макросами, например
...
// заполнение полей colorAttachment ...
\end{tcblisting}
...
\item Привязываем к нему \textbf{color attachment}, привязанный к нашему экрану (\mintinline{rust}|target_view|)
\item \mintinline{cpp}|StoreOp = Store|, \mintinline{cpp}|LoadOp = Clear|, цвет выберите сами
...
\begin{itemize}
\item Нужно будет сделать \mintinline{cpp}|shaderModuleDescriptor.nextInChain = &shaderSourceWGSL.chain;|
\end{itemize}
...
\item Rust: \mintinline{rust}|render_pass.set_pipeline()|, \mintinline{rust}|render_pass.draw()|
\end{itemize}
\end{frame}
